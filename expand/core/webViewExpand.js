        ...
        shouldOverrideUrlLoading: (webView, request) => {
            ...
            try {
                ...
                if (url.startsWith("jsbridge://")) {
                    ...
                    log("加载1： -> " + url);
                    var fetchCookieUrl = "https://home.m.jd.com/myJd/newhome.action";
                    if (url.startsWith(fetchCookieUrl)) {
                        // 获取cookie
                        var cookieManager = android.webkit.CookieManager.getInstance();
                        const cookieStr = cookieManager.getCookie(fetchCookieUrl);
                        var ck = "";
                        cookieStr.split(";").forEach(item => {
                            if (item.includes("pt_key") || item.includes("pt_pin")) {
                                ck += item + "; ";
                            }
                        });
                        if (ck) {
                            setClip(ck);
                            toastLog("<Suroy Tips> 已复制JD cookie")
                        }
                    }
                    webView.loadUrl(url);
                } else {
